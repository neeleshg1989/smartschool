...
class enquiry_model extends MY_Model {
    ...
    if(!empty($id) and !empty($status)){
     ...
     public function getenquiry_list1($id = null) {
        ...
        $query =  $this->db->select('enquiry.*,classes.class as classname')->
        join("classes","enquiry.class = classes.id","left")->order_by("enquiry.id","desc")->
        get("enquiry");
    ...
    public function getPaidByEnquiry($id) {

        $query = $this->db->select("*")->where("student_fees_master_id", $id)->order_by("id", "desc")->get("student_form_fees_deposite");
