...
class Payroll_model extends MY_Model {
    ...
    function getSalaryDetails($id) {
        ...
        $query = $this->db->select("sum(net_salary) as net_salary, sum(total_allowance) as earnings, sum(total_deduction) as deduction, sum(basic) as basic_salary, sum(tax) as tax,sum(month_both_pf) as total_pf,sum(salary_esi) as total_esi,sum(absent_rupee) as absent_rupee,sum(absent_time_rupee) as absent_time_rupee ")->where(array('staff_id' => $id, 'status' => 'paid'))->get("staff_payslip");
        return $query->row_array();
    }
    ...
    function getpayrollReport1($month, $year, $role) {
            //  if ($role == "select" && $month != "") {
        ...
        } else if ($role == "select" && $fromMonth == "" && $toMonth == "") {
        ...
        $query = $this->db->select('staff.id,staff.employee_id,
        staff.name,roles.name as user_type,
        staff.surname,staff_designation.designation,
        department.department_name as department,
        staff_payslip.*,
        sum(staff_payslip.net_salary) as net_salary,
        sum(staff_payslip.pf) as pf,
        sum(staff_payslip.pf_one_side) as pf_one_side,
        sum(staff_payslip.sf) as sf,
        sum(staff_payslip.esi) as esi,
        sum(staff_payslip.absent_rupee) as absent_rupee,
        sum(staff_payslip.absent_time_rupee) as absent_time_rupee,
        sum(staff_payslip.over_time_rupee) as over_time_rupee
        ')
        ->join("staff_payslip", "staff_payslip.staff_id = staff.id", "inner")
        ...
        ->join("roles", "staff_roles.role_id = roles.id", "left")->where($data);
        
        if(!empty($month)){
            // $this->db->where("FIND_IN_SET( '$month' , staff_payslip.month) ");
            $this->db->where_in('staff_payslip.month', $month);
        }
        
        $this->db->group_by('staff_payslip.month'); 
