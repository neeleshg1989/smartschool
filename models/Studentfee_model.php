...
class Studentfee_model extends CI_Model {
    ...
    public function remove_transport_fee($id, $sub_invoice) {
        $this->db->where('id', $id);
        $q = $this->db->get('student_transport_fees_deposite');
        if ($q->num_rows() > 0) {
            $result = $q->row();
            ...
            if (!empty($a)) {
                $data['amount_detail'] = json_encode($a);
                $this->db->where('id', $id);
                $this->db->update('student_transport_fees_deposite', $data);
            } else {
                $this->db->where('id', $id);
                $this->db->delete('student_transport_fees_deposite');
            }
        }
    }
    
    public function remove_other_fee($id, $sub_invoice) {
        $this->db->where('id', $id);
        ...
        if ($q->num_rows() > 0) {
            ...
            if (!empty($a)) {
                $data['amount_detail'] = json_encode($a);
                $this->db->where('id', $id);
                $this->db->update('student_other_fees_deposite', $data);
            } else {
                $this->db->where('id', $id);
                $this->db->delete('student_other_fees_deposite');
            }
        }
    }
    
     public function remove_form_fee($id, $sub_invoice) {
        $this->db->where('id', $id);
        ...
        if ($q->num_rows() > 0) {
            ...
            if (!empty($a)) {
                $data['amount_detail'] = json_encode($a);
                $this->db->where('id', $id);
                $this->db->update('student_form_fees_deposite', $data);
            } else {
                $this->db->where('id', $id);
                $this->db->delete('student_form_fees_deposite');
            }
        }
    }
    ...
    public function getDueStudentFees1($class_id) {

        if($class_id=='all'){
            $query = "SELECT IFNULL(student_fees_deposite.id, 0) as student_fees_deposite_id, 
            ...
            `students`.`firstname`, `students`.`lastname`, `students`.`image`, `students`.`mobileno`, 
            ...
            `students`.`ifsc_code`, `students`.`guardian_name`, `students`.`guardian_relation`, `students`.`guardian_phone`, 
            `students`.`guardian_address`, `students`.`is_active`, `students`.`created_at`, `students`.`updated_at`, 
            ...
            INNER JOIN student_fees_master on student_fees_master.student_session_id=student_session.id  
            ...
            WHERE `student_session`.`session_id` = " . $this->current_session . "  AND  `students`.`is_active` = 'yes'  ORDER BY `students`.`id`";
        }else{
            ...
            WHERE `student_session`.`session_id` = " . $this->current_session . " AND `student_session`.`class_id` = " . $this->db->escape($class_id) . " AND  `students`.`is_active` = 'yes'  ORDER BY `students`.`id`";
        }
    ...
    public function getDueStudentBusFees($class_id) {
        if($class_id=='all'){
            $query = "SELECT 
            IFNULL(student_transport_fees_deposite.id, 0) as student_transport_fees_deposite_id, 
            IFNULL(student_transport_fees_deposite.fee_groups_feetype_id, 0) as fee_groups_feetype_id, 
            IFNULL(student_transport_fees_deposite.amount_detail, 0) as amount_detail, 
            student_transport_fees_master.id as `fee_master_id`,
            transport_fee_groups_feetype.feetype_id ,
            transport_fee_groups_feetype.amount,
            transport_fee_groups_feetype.due_date, 
            `classes`.`id` AS `class_id`, `student_session`.`id` as `student_session_id`, 
            `students`.`id`, `classes`.`class`, `sections`.`id` AS `section_id`, 
            `sections`.`section`, `students`.`id`, 
            ...
            INNER JOIN student_transport_fees_master on student_transport_fees_master.student_session_id=student_session.id  
            
            LEFT JOIN student_transport_fees_deposite on student_transport_fees_deposite.student_fees_master_id=student_transport_fees_master.id   
            
            INNER JOIN transport_fee_groups_feetype on transport_fee_groups_feetype.id = student_transport_fees_deposite.fee_groups_feetype_id 
